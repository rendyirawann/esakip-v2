                ...
                <div class="card">
                    <div class="card-header" style="background-color: #04A9F5; padding: 8px;">
                        <h6 style="color: white; margin: 0; cursor: pointer;" id="toggleAll">
                            <i class="fas fa-pen-fancy"></i>Periode Indikator Tujuan Renstra - <?= Html::decode(ucwords(strtolower($nama_skpd))) ?>
                        </h6>
                    </div>
                    <div class="card-body">
                        <div class="row">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mt-4">
                            ...
                            <?php else: ?>
                                <?php foreach ($groupedData as $misi): ?>
                                    <div class="card mb-4 shadow-sm">
                                        <div class="card-header bg-primary text-white">
                                            <h5 class="mb-0 text-uppercase"><i class="fas fa-flag-checkered me-2"></i>Misi:</h5>
                                            <p class="mb-0 ms-4"><?= Html::decode($misi['uraian']) ?></p>
                                        </div>
                                        <div class="card-body">
                                            <?php foreach ($misi['tujuans'] as $tujuan): ?>
                                                <div class="tujuan-block mb-4">
                                                    ...
                                                    <?php foreach ($tujuan['sasarans'] as $sasaran): ?>
                                                        <div class="p-3 border rounded mb-3" style="background-color: #f8f9fa;">
                                                            ...
                                                            <?php if (empty($sasaran->tujuanRenstra)): ?>
                                                                <div class="alert alert-light text-center p-2"><i>Belum ada Tujuan Renstra untuk sasaran ini.</i></div>
                                                            <?php else: ?>
                                                                <?php foreach ($sasaran->tujuanRenstra as $tujuanRenstra): ?>
                                                                    <div class="list-group-item list-group-item-light p-3 mb-2 rounded">
                                                                        <div class="d-flex w-100 justify-content-between">
                                                                            <h6 class="mb-1 fw-bold text-success">Tujuan Renstra: <?= Html::decode($tujuanRenstra->uraian_tujuanrenstra) ?></h6>
                                                                            <?= Html::button('Tambah Data Indikator Tujuan Renstra', [
                                                                                'class' => 'btn btn-success mb-2',
                                                                                'data-bs-toggle' => 'modal',
                                                                                'data-bs-target' => '#createModal',
                                                                                'data-refsasaranrenstra' => $tujuanRenstra->refsasaranrenstra_id, // Mengambil refsasaranrenstra_id
                                                                                'data-reftujuanrenstra' => $tujuanRenstra->reftujuanrenstra_id, // Mengambil reftujuanrenstra_id
                                                                                'data-refperiode' => $tujuanRenstra->refperiode_id // Mengambil refperiode_id
                                                                            ]) ?>
                                                                        </div>
                                                                        <hr class="my-2">
                                                                        <ul class="list-unstyled ps-3">
                                                                            <?php if (empty($tujuanRenstra->indikatorTujuanRenstra)): ?>
                                                                                <li class="font-italic text-muted">Belum ada Indikator.</li>
                                                                            <?php else: ?>
                                                                                <?php foreach ($tujuanRenstra->indikatorTujuanRenstra as $indikator): ?>
                                                                                    <li class="d-flex justify-content-between align-items-center py-1">
                                                                                        <span><i class="fas fa-caret-right me-2 text-muted"></i><?= Html::decode($indikator->uraian_indikatortujuanrenstra) ?></span>
                                                                                        <div class="btn-group" role="group">
                                                                                            <?= Html::button('<i class="fas fa-edit"></i>', [
                                                                                                ...
                                                                                                'data-url' => Url::to(['update', 'refindikatortujuanrenstra_id' => $indikator->refindikatortujuanrenstra_id])
                                                                                            ]) ?>
                                                                                            <?= Html::a('<i class="fas fa-trash-alt"></i>', ['delete', 'refindikatortujuanrenstra_id' => $indikator->refindikatortujuanrenstra_id], [
