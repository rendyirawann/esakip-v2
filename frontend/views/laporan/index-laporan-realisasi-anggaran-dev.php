                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-sm-12">
                                ...
                                <?= \yii\helpers\Html::beginForm(['index-laporan-realisasi-anggaran-dev'], 'get', ['class' => 'form-inline']); ?>
                            ...
                            <div class="col-sm-12">
                                <div class="btn-group mb-3" role="group">
                                    ...
                                    <?= Html::a(
                                        ...
                                        ['cetak-realisasi-anggaran', 'refperiode_id' => $selectedPeriodId, 'refskpd_id' => $selectedSkpdId],
                                        ['class' => 'btn btn-danger', 'target' => '_blank', 'data-pjax' => 0]
                                    ) ?>
                                    <?= Html::a(
                                        '<i class="fas fa-file-excel"></i> Excel',
                                        ['cetak-realisasi-anggaran-excel', 'refperiode_id' => $selectedPeriodId, 'refskpd_id' => $selectedSkpdId],
                                            ...
                                            <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tbdata">
                                                ...
                                                <tbody>
                                                    ...
                                                    foreach ($data as $refsasaranrenstra_id => $entry): ?>
                                                        <tr>
                                                            <td colspan="3"><?= Html::encode($entry['uraian_sasaran']) ?></td>
                                                        </tr>
                                                        <?php foreach ($entry['programs'] as $programId => $program): ?>
                                                            <tr>
                                                                <td align='center'><?= $no++ ?></td>
                                                                <td><?= Html::encode($program['nama_program']) ?></td>
                                                                <td align='right'>Rp. <?= Html::encode(number_format((float) $program['anggaran_pk_p'], 2)) ?></td>

                                                                <?php for ($i = 0; $i < 4; $i++): ?>
                                                                    <td align='right'>
                                                                        Rp. <?= isset($program['quarterly'][$i]) ? Html::encode(number_format($program['quarterly'][$i]['triwulan_penyerapan_anggaran'], 2)) : '0' ?>
                                                                    </td>
                                                                    <td align='center'>
                                                                        <?php 
                                                                            $penyerapan = isset($program['quarterly'][$i]) ? (float)$program['quarterly'][$i]['triwulan_penyerapan_anggaran'] : 0;
                                                                            $pagu = (float)$program['anggaran_pk_p'];
                                                        ...
                                                        <?php endforeach; ?>
                                                        <tr>
                                                            <td colspan="2" align="right"><strong>Total Anggaran:</strong></td>
                                                            <td align='right'>Rp. <?= Html::encode(number_format($entry['total_anggaran'], 2)) ?></td>
                                                            <?php for ($i = 0; $i < 4; $i++): ?>
                                                                <td align='right'>
                                                                    Rp. <?= Html::encode(number_format($entry['total_quarterly_penyerapan_anggaran'][$i], 2)) ?>
                                                                </td>
                                                                <td align='center'>
                                                                    <?php 
                                                                        $penyerapanTotal = (float)($entry['total_quarterly_penyerapan_anggaran'][$i] ?? 0);
                                                                        $paguTotal = (float)$entry['total_anggaran'];
