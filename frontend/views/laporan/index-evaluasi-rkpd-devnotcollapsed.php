...
$this->registerJs("
    $('button[data-bs-toggle=\"collapse\"]').on('click', function() {
        ...
        // Langsung toggle ikon pada tombol yang diklik
        icon.toggleClass('fa-plus fa-minus');
    });

    // Saat sebuah item ditutup, cari tombol yang menargetkannya dan pastikan ikonnya '+'
    $('.collapse').on('hide.bs.collapse', function () {
        var targetSelector = '.' + $(this).attr('class').split(' ').join('.');
        $('button[data-bs-target=\"' + targetSelector + '\"]').find('i.fas').removeClass('fa-minus').addClass('fa-plus');
    });
");
                                ...
                                <div class="btn-group mb-3" role="group">
                                    ...
                                    <div class="btn-group">
                                        <?= Html::a(
                                            ...
                                            ['cetak-evaluasi-rkpd', 'refperiode_id' => $selectedPeriodId, 'refskpd_id' => $selectedSkpdId],
                                            ['class' => 'btn btn-danger', 'target' => '_blank', 'data-pjax' => 0]
                                        ) ?>

                                        <?= Html::a(
                                            '<i class="fas fa-file-excel"></i> Cetak Excel',
                                            ['cetak-evaluasi-rkpd-excel', 'refperiode_id' => $selectedPeriodId, 'refskpd_id' => $selectedSkpdId],
                                            ...
                                            <table class="table table-bordered text-center">
                                                ...
                                                <tbody>
                                                    <?php if (empty($laporanData)): ?>
                                                        <tr>
                                                            <td colspan="14" class="text-center"><i>Tidak ada data untuk ditampilkan.</i></td>
                                                        </tr>
                                                    <?php else: ?>
                                                        <?php foreach ($laporanData as $program): ?>
                                                            <tr class="table-light text-start">
                                                                <td class="fw-bold align-middle"><?= Html::encode($program['kode_program']) ?></td>
                                                            ...
                                                            <?php if (!empty($program['kegiatans'])): ?>
                                                                <?php foreach ($program['kegiatans'] as $kegiatan): ?>
                                                                    <tr class="text-start" style="background-color: #f8f9fa;">
                                                                        <td class="ps-4 align-middle"><?= Html::encode($kegiatan['kode_kegiatan']) ?></td>
                                                                    ...
                                                                    <?php if (!empty($kegiatan['subkegiatans'])): ?>
                                                                        ...
                                                                        <?php endforeach; // Penutup untuk $subkegiatan 
                                                                        ?>
                                                                    <?php endif; // Penutup untuk if !empty($kegiatan['subkegiatans']) 
                                                                    ?>
                                                                <?php endforeach; // Penutup untuk $kegiatan 
                                                                ?>
                                                            <?php endif; // Penutup untuk if !empty($program['kegiatans']) 
                                                            ?>
                                                        <?php endforeach; // Penutup untuk $program 
                                                        ?>
                                                    <?php endif; // Penutup untuk if empty($laporanData) 
                                                    ?>
                                                </tbody>
                                            </table>
                                        </div>
