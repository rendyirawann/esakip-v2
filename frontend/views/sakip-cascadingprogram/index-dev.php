                        ...
                        <?php else: ?>
                            <div class="dt-responsive table-responsive">
                                <table id="table-style-hover" class="table table-striped table-hover table-bordered nowrap" style="font-size:xx-small;">
                                    ...
                                    ?>
                                    <?php foreach ($cascadingPrograms as $cascadingProgram): ?>
                                        <?php if ($currentRefsasaranrenstraId !== $cascadingProgram->refsasaranrenstra_id): ?>
                                            <tr>
                                                <th colspan="<?= $isAdmin ? 6 : 5 ?>" style="background-color: #04A9F5; color:white; white-space: normal;"><?= $cascadingProgram->refmisi->uraian_misi ?></th>
                                            </tr>
                                            <tr>
                                                <th colspan="<?= $isAdmin ? 6 : 5 ?>" style="background-color: #e23c3c; color:white; white-space: normal;">(Tujuan)<?= $cascadingProgram->reftujuan->uraian_tujuan ?></th>
                                            </tr>
                                            <tr>
                                                <th colspan="<?= $isAdmin ? 6 : 5 ?>" style="background-color: #6ee15b; color:white; white-space: normal;">(Sasaran)<br><?= $cascadingProgram->sasaranRenstra->uraian_sasaranrenstra ?></th>
                                            </tr>
                                            <?php $currentRefsasaranrenstraId = $cascadingProgram->refsasaranrenstra_id; ?>
                                        <?php endif; ?>

                                        <?php if ($currentRefprogramId !== $cascadingProgram->refprogram_id): ?>
                                            <tr>
                                                <?php if ($isAdmin): ?>
                                                    <th style="background-color: #16537e; color:white;">Aksi</th>
                                                <?php endif; ?>
                                                <th style="background-color: #16537e; color:white;"><?= $cascadingProgram->refProgram->kode_program ?></th>
                                                <th style="background-color: #16537e; color:white; white-space: normal;"><?= $cascadingProgram->refProgram->nama_program ?></th>
                                                <th style="background-color: #16537e; color:white;">Satuan</th>
                                                <th style="background-color: #16537e; color:white;"><?= $cascadingProgram->refPeriode->periode ?></th>
                                                <th style="background-color: #16537e; color:white;">Anggaran</th>
                                            </tr>
                                            <?php $currentRefprogramId = $cascadingProgram->refprogram_id;
                                            // $totalAnggaran = 0;

                                            // Calculate total anggaran for the current refprogram_id
                                            $totalAnggaran = SakipCascadingsubkegiatan::find()
                                                ->where(['refprogram_id' => $cascadingProgram->refprogram_id])
                                                ->andWhere(['refskpd_id' => $cascadingProgram->refskpd_id])
                                            ...
                                            <?php if ($isAdmin): ?>
                                                <td class="text-center">
                                                    <?= Html::button('<i class="fas fa-edit"></i>', [
                                                        ...
                                                        'data-url' => Url::to(['update', 'refcascadingprogram_id' => $cascadingProgram->refcascadingprogram_id])
                                                    ]) ?>
                                                </td>
                                            <?php endif; ?>
                                            <td>Sasaran/Indikator</td>
                                            <td style="white-space: normal"><?= $cascadingProgram->uraian_sasaranprogram ?> - <?= $cascadingProgram->uraian_indikatorprogram ?></td>
                                            <td><?= $cascadingProgram->program_satuan ?></td>
                                            <td><?= $cascadingProgram->program_target ?></td>
                                        ...
                                        <tr>
                                            <td colspan="<?= $isAdmin ? 6 : 5 ?>">
                                                <div class="accordion" id="accordion<?= $cascadingProgram->refcascadingprogram_id ?>">
                                                    <div class="accordion-item">
                                                        <h2 class="accordion-header" id="heading<?= $cascadingProgram->refcascadingprogram_id ?>">
                                                            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse<?= $cascadingProgram->refcascadingprogram_id ?>" aria-expanded="false" aria-controls="collapse<?= $cascadingProgram->refcascadingprogram_id ?>">
                                                                Lihat Penjabat SKPD
                                                            </button>
                                                        </h2>
                                                        <div id="collapse<?= $cascadingProgram->refcascadingprogram_id ?>" class="accordion-collapse collapse" aria-labelledby="heading<?= $cascadingProgram->refcascadingprogram_id ?>" data-bs-parent="#accordion<?= $cascadingProgram->refcascadingprogram_id ?>">
                                                            <div class="accordion-body">
                                                                <?php
                                                                $penjabatModels = SakipPenjabatskpdCascadingprogram::findAll(['refcascadingprogram_id' => $cascadingProgram->refcascadingprogram_id]);
