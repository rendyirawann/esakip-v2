...
$this->title = 'e-Sakip - Data Cascading Program';
            ...
            <div class="col-sm-12">
                ...
                <div class="card">
                    ...
                    <div class="card-body" id="refresh">
                        ...
                        <?= Html::button('<i class="fas fa-plus-square"></i> Data Cascading Program', [
                            'class' => 'btn btn-success mb-2',
                            'id' => 'btn-create-program',
                        ]) ?>
                        <?= Html::a('<i class="fas fa-sync"></i> Refresh', ['index'], ['class' => 'btn btn-primary mb-2']) ?>
                        <?= Html::a('<i class="fas fa-file-excel"></i> Export to Excel', ['export-excel', 'refperiode_id' => $selectedPeriodId], ['class' => 'btn btn-success mb-2']) ?>
                        <?= Html::a('<i class="fas fa-file-pdf"></i> Export to PDF', ['export-pdf', 'refperiode_id' => $selectedPeriodId], ['class' => 'btn btn-danger mb-2']) ?>
                        <?= Html::a('<i class="fas fa-print"></i> Print', ['print', 'refperiode_id' => $selectedPeriodId], ['class' => 'btn btn-info mb-2']) ?>
                        <?= Html::a('<i class="fas fa-file-pdf"></i> Export to Word', ['export-word', 'refperiode_id' => $selectedPeriodId], ['class' => 'btn btn-warning mb-2']) ?>
                        <?= Html::a('<i class="fas fa-file-pdf"></i> Export to CSV', ['export-csv', 'refperiode_id' => $selectedPeriodId], ['class' => 'btn btn-secondary mb-2']) ?>
                        <?= Html::a('<i class="fas fa-file-pdf"></i> Export to JSON', ['export-json', 'refperiode_id' => $selectedPeriodId], ['class' => 'btn btn-dark mb-2']) ?>
                        <?= Html::a('<i class="fas fa-file-pdf"></i> Export to XML', ['export-xml', 'refperiode_id' => $selectedPeriodId], ['class' => 'btn btn-light mb-2']) ?>
                        <?= Html::a('<i class="fas fa-file-pdf"></i> Export to HTML', ['export-html', 'refperiode_id' => $selectedPeriodId], ['class' => 'btn btn-info mb-2']) ?>
                        <?= Html::a('<i class="fas fa-file-pdf"></i> Export to Markdown', ['export-markdown', 'refperiode_id' => $selectedPeriodId], ['class' => 'btn btn-secondary mb-2']) ?>
                        <?= Html::a('<i class="fas fa-file-pdf"></i> Export to LaTeX', ['export-latex', 'refperiode_id' => $selectedPeriodId], ['class' => 'btn btn-primary mb-2']) ?>
                        ...
                        <?php else: ?>
                            <div class="dt-responsive table-responsive">
                                <table id="table-style-hover" class="table table-striped table-hover table-bordered nowrap" style="font-size:xx-small;">
                                    ...
                                    $totalAnggaran = 0; // Initialize total anggaran
                                    ?>
                                    <?php foreach ($dataProvider->getModels() as $model): ?>
                                        <?php if ($currentRefsasaranrenstraId !== $model->refsasaranrenstra_id): ?>
                                            <tr>
                                                <th colspan="6" style="background-color: #04A9F5; color:white; white-space: normal;"><?= isset($model->refmisi) ? $model->refmisi->uraian_misi : '-' ?></th>
                                            </tr>
                                            <tr>
                                                <th colspan="6" style="background-color: #e23c3c; color:white; white-space: normal;">(Tujuan)<?= isset($model->reftujuan) ? $model->reftujuan->uraian_tujuan : '-' ?></th>
                                            </tr>
                                            <tr>
                                                <th colspan="6" style="background-color: #6ee15b; color:white; white-space: normal;">(Sasaran) <br><?= isset($model->sasaranRenstra) ? $model->sasaranRenstra->uraian_sasaranrenstra : '-' ?></th>
                                            </tr>
                                        ...
                                        <?php if ($currentRefprogramId !== $model->refprogram_id): ?>
                                            <tr>
                                                ...
                                                <th style="background-color: #16537e; color:white;">Target <?= $model->refPeriode->periode ?></th>
                                        ...
                                        <tr>
                                            <td>
                                                <?= Html::button('<i class="fas fa-edit"></i>', [
                                                    'class' => 'btn btn-success btn-sm btn-edit-program',
                                                    'title' => 'Update',
                                                    'data-url' => Url::to(['update', 'refcascadingprogram_id' => $model->refcascadingprogram_id])
                                                ]) ?>

                                                <?= Html::a('<i class="fas fa-trash-alt"></i>', ['delete', 'refcascadingprogram_id' => $model->refcascadingprogram_id], [
                                                ...
                                                <?= Html::button('<i class="fas fa-check"></i>', [
                                                    ...
                                                    'data-refbidang_id' => $model->refbidang_id,
                                                    'data-refprogram_id' => $model->refprogram_id,
                                                    'data-uraian_sasaranprogram' => $model->uraian_sasaranprogram,
                                                    'data-uraian_indikatorprogram' => $model->uraian_indikatorprogram,
                                                    'data-program_target' => $model->program_target,
                                                    'data-program_satuan' => $model->program_satuan,
                                                ]) ?>
                                            </td>
                                        ...
                                        <tr>
                                            <td colspan="6">
                                                <div class="accordion" id="accordion<?= $model->refcascadingprogram_id ?>">
                                                    <div class="accordion-item">
                                                        <h2 class="accordion-header" id="heading<?= $model->refcascadingprogram_id ?>">
                                                            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse<?= $model->refcascadingprogram_id ?>" aria-expanded="false" aria-controls="collapse<?= $model->refcascadingprogram_id ?>">
                                                                Lihat Penjabat SKPD
                                                            </button>
                                                        </h2>
                                                        <div id="collapse<?= $model->refcascadingprogram_id ?>" class="accordion-collapse collapse" aria-labelledby="heading<?= $model->refcascadingprogram_id ?>" data-bs-parent="#accordion<?= $model->refcascadingprogram_id ?>">
                                                            <div class="accordion-body">
                                                                <?php
                                                                $penjabatModels = SakipPenjabatskpdCascadingprogram::findAll(['refcascadingprogram_id' => $model->refcascadingprogram_id]);
                                                                foreach ($penjabatModels as $penjabatModel): ?>
                                                                    ...
                                                                    <?= Html::a('<i class="fas fa-trash-alt"></i> Hapus Penjabat SKPD', [
                                                                        'sakip-penjabatskpd-cascadingprogram/delete',
                                                                        'refpenjabatcascadingprogram_id' => $penjabatModel->refpenjabatcascadingprogram_id,
