...
$this->registerJs("
    $('#dataindikatorsasaranrenstra').on('change', '#refsasaranrenstra_p_id', function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.success) {
                    $('#refperiode_p_id').val(data.periode); // Update input text dengan periode
                    $('#refperiode_p_id_hidden').val(data.refperiode_p_id); // Update hidden input dengan refperiode_id
                } else {
                    $('#refperiode_p_id').val('Periode tidak ditemukan'); // Jika tidak ditemukan
                    $('#refperiode_p_id_hidden').val(''); // Kosongkan hidden input
                }
            },
            error: function() {
                $('#refperiode_p_id').val('Terjadi kesalahan'); // Jika terjadi kesalahan
                $('#refperiode_p_id_hidden').val(''); // Kosongkan hidden input
            }
        });
    });
...
<div class="sakip-indikatorsasaranrenstra-form">
    ...
    <?= $form->field($model, 'uraian_indikatorsasaranrenstra_p')->textarea(['rows' => 2]) ?>

    <?= $form->field($model, 'refskpd_id')->hiddenInput(['value' => $userRefSkpdId])->label(false) ?>

    <?= $form->field($model, 'refsasaranrenstra_p_id')->hiddenInput(['maxlength' => true])->label(false) ?>

    <?= $form->field($model, 'refperiode_id')->hiddenInput(['id' => 'refperiode_id_hidden'])->label(false) ?>

    <div class="row">
        <div class="col-sm-6">
            <?= $form->field($model, 'indikatorsasaranrenstra_p_target')->textInput(['maxlength' => true])->label('Target') ?>
        </div>
        <div class="col-sm-6">
            <?= $form->field($model, 'indikatorsasaranrenstra_p_satuan')->textInput(['maxlength' => true])->label('Satuan') ?>
        </div>
    ...
    <?= $form->field($model, 'indikatorsasaranrenstra_p_isaktif')->radioList(
    ...
    )->label('Status Indikator Sasaran Renstra Perubahan') ?>

    <?= $form->field($model, 'iku_isaktif')->radioList(
        ['T' => 'Aktif', 'F' => 'Tidak Aktif'],
        [
            'item' => function ($index, $label, $name, $checked, $value) {
                return '<label class="radio-inline">' . Html::radio($name, $checked, ['value' => $value]) . $label . '</label>';
    ...
    <?= $form->field($model, 'pk_isaktif')->radioList(
        ['T' => 'Aktif', 'F' => 'Tidak Aktif'],
        [
            'item' => function ($index, $label, $name, $checked, $value) {
                return '<label class="radio-inline">' . Html::radio($name, $checked, ['value' => $value]) . $label . '</label>';
