...
$this->registerJs("
    ...
    $(document).ready(function() {
        ...
        $('form#laporanrenjakatapengantar').off('submit').on('submit', function(e) {
...
<div class="laporan-renja-kata-pengantar-form">

    <?php
    $form = ActiveForm::begin([
        'id' => 'laporanrenjakatapengantar',
        'options' => ['enctype' => 'multipart/form-data'],
    ]);
    ?>

    <?= $form->field($model, 'uraian_katapengantar')->widget(CKEditor::className(), [
        'options' => ['rows' => 7],
        'preset' => 'full'
    ]); ?>

    <?= $form->field($model, 'refperiode_id')->dropDownList(
        \yii\helpers\ArrayHelper::map(\frontend\models\SakipPeriode::find()->all(), 'refperiode_id', 'periode'),
        ['prompt' => 'Pilih Periode RENJA']
    ) ?>
    ...
    <?= $form->field($model, 'halaman_renja')->textInput(['maxlength' => true]) ?>
