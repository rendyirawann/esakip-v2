    ...
    $(document).ready(function() {
        ...
        $('form#dataindikatorcascadingsubkegiatantriwulan').off('submit').on('submit', function(e) {
            ...
            var \$submitBtn = \$form.find(':submit'); // Temukan tombol submit
                       ...
                       // Format triwulan_penyerapan_anggaran before submission
            var anggaranField = $('#sakipindikatorcascadingsubkegiatantriwulan-triwulan_penyerapan_anggaran');
...
$this->registerJs("
   // Add new JavaScript for formatting the anggaran input
    $(document).ready(function() {
        $('#sakipindikatorcascadingsubkegiatantriwulan-triwulan_penyerapan_anggaran').on('input', function() {
            let value = $(this).val();
...
<div class="sakip-indikatorcascadingsubkegiatan-triwulan-form">
    ...
    <div class="row">
        ...
        <div class="col-lg-6">
            <?= $form->field($model, 'triwulan_penyerapan_anggaran')->textInput(['maxlength' => true, 'value' => $model->triwulan_penyerapan_anggaran ? number_format($model->triwulan_penyerapan_anggaran, 0, ',', '.') : ''])->label('Penyerapan Anggaran') ?>
        </div>
    </div>
    ...
    <?= $form->field($model, 'triwulan_realisasi')->hiddenInput(['id' => 'triwulan_realisasi', 'maxlength' => true])->label(false) ?>

    <!-- Triwulan Capaian: Auto-calculated and displayed as percentage -->
    <?= $form->field($model, 'triwulan_capaian')->hiddenInput(['id' => 'triwulan_capaian', 'maxlength' => true, 'readonly' => true])->label(false) ?>

    <?= $form->field($model, 'triwulan_keterangan')->hiddenInput(['rows' => 2])->label(false) ?>
