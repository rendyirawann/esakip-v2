...
$this->registerJs("
 ...
 // Event handler untuk dropdown PERIODE
    $('#ref-periode-dropdown').change(function() {
        var refPeriodeId = $(this).val();
        var kegiatanDropdown = $('#refkegiatan-dropdown');

        // PERBAIKAN: Hancurkan instance Selectize pada dropdown anak sebelum di-update
        if (kegiatanDropdown[0].selectize) {
            kegiatanDropdown[0].selectize.destroy();
        }
        // Juga hancurkan dan reset dropdown di level bawahnya
        ...
        if ($('#refsubkegiatan-dropdown')[0].selectize) {
            $('#refsubkegiatan-dropdown')[0].selectize.destroy();
        }
        $('#refkegiatan-indicator-dropdown').html('<option value=\'\'>Pilih Indikator Kegiatan</option>').selectize();
        $('#refsubkegiatan-dropdown').html('<option value=\'\'>Pilih Sub Kegiatan</option>').selectize();
        ...
        if(refPeriodeId) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if(response.success) {
                        kegiatanDropdown.html(response.dropdown).selectize({
                            create: false,
                            sortField: 'text',
                        ...
                        kegiatanDropdown.html('<option value=\'\'>Pilih Kegiatan</option>').selectize();
                    }
                },
                error: function() {
...
$('#refkegiatan-dropdown').change(function() {
    var refKegiatanId = $(this).val();
     ...
     // Hancurkan dan reset dropdown di level bawahnya
        if ($('#refkegiatan-indicator-dropdown')[0].selectize) {
            $('#refkegiatan-indicator-dropdown')[0].selectize.destroy();
        }
        if ($('#refsubkegiatan-dropdown')[0].selectize) {
        ...
        $('#refkegiatan-indicator-dropdown').html('<option value=\'\'>Pilih Indikator Kegiatan</option>');
        $('#refsubkegiatan-dropdown').html('<option value=\'\'>Pilih Sub Kegiatan</option>');
        
        // Reset field-field terkait
        $('#refcascadingprogram_id, #sakip-cascadingkegiatan-refsasaranrenstra_id, #refsasaranrenstra_id_display, #sakip-cascadingkegiatan-refindikatorsasaranrenstra_id, #refindikatorsasaranrenstra_id_display').val('');

    
    
    if(refKegiatanId && refPeriodeId) {
        showLoading();
        ...
        $.ajax({
            ...
            type: 'POST',
            data: { refkegiatan_id: refKegiatanId, refperiode_id: refPeriodeId },
            success: function(response) {
                if(response.success) {
