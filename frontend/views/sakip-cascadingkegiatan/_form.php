...
$('#ref-periode-dropdown').change(function() {
    ...
    var programDropdown = $('#refprogram-dropdown');

        // PERBAIKAN: Hancurkan instance Selectize yang ada sebelum mengganti HTML
    ...
    if(refPeriodeId) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if(response.success) {
                    // Ganti konten dropdown dan inisialisasi ulang Selectize
                        programDropdown.html(response.dropdown);
                        programDropdown.selectize(); // Inisialisasi ulang
                        
                        $('#additional-fields').show();
                } else {
                    alert('Error: ' + response.message);
                        programDropdown.html('<option value=\'\'>Pilih Program</option>'); // Kosongkan jika error
...
$('#refprogram-dropdown').change(function() {
    ...
    if ($('#refkegiatan-dropdown')[0].selectize) {
     ...
     // Kosongkan dropdown anak
        $('#refprogram-indicator-dropdown').html('<option value=\'\'>Pilih Indikator Program</option>');
        $('#refkegiatan-dropdown').html('<option value=\'\'>Pilih Kegiatan</option>');
        
    if(refProgramId && refPeriodeId) {
    // Reset refsasaranrenstra_id when program is changed
        ...
        $.ajax({
            ...
            success: function(response) {
                if(response.success) {
                    $('#refprogram-indicator-dropdown').html(response.indicatorDropdown).selectize();;
 ...
 $('#refprogram-indicator-dropdown').change(function() {
    ...
    function fetchIndikatorsasaranrenstra(refCascadingProgramId) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if(response.success) {
                    $('#refkegiatan-dropdown').html(response.dropdown).selectize();;
                   
                } else {
                    alert('Error: ' + response.message);
                $('#refkegiatan-dropdown').selectize(); // Inisialisasi ulang meskipun kosong
                }
                hideLoading();
            },
            error: function() {
