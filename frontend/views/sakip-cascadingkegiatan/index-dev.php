                        ...
                        <?php else: ?>
                            <div class="dt-responsive table-responsive">
                                <table id="table-style-hover" class="table table-striped table-hover table-bordered nowrap" style="font-size:xx-small;">
                                    ...
                                    <?php foreach ($cascadingKegiatans as $cascadingKegiatan): ?>
                                        <!-- Check if the refsasaranrenstra_id has changed to display the mission, goal, and target -->
                                        <?php if ($currentRefsasaranrenstraId !== $cascadingKegiatan->refCascadingProgram->refsasaranrenstra_id): ?>
                                            <tr>
                                                <th colspan="<?= $isAdmin ? 6 : 5 ?>" style="background-color: #04A9F5; color:white; white-space: normal;"><?= $cascadingKegiatan->refCascadingProgram->refmisi->uraian_misi ?></th>
                                            </tr>
                                            <tr>
                                                <th colspan="<?= $isAdmin ? 6 : 5 ?>" style="background-color: #e23c3c; color:white; white-space: normal;">(Tujuan)<?= $cascadingKegiatan->refCascadingProgram->reftujuan->uraian_tujuan ?></th>
                                            </tr>
                                            <tr>
                                                <th colspan="<?= $isAdmin ? 6 : 5 ?>" style="background-color: #6ee15b; color:white; white-space: normal;">(Sasaran)<br><?= $cascadingKegiatan->refCascadingProgram->sasaranRenstra->uraian_sasaranrenstra ?></th>
                                            </tr>
                                            <?php $currentRefsasaranrenstraId = $cascadingKegiatan->refCascadingProgram->refsasaranrenstra_id; ?>
                                        <?php endif; ?>

                                        <!-- Check if the cascading program has changed to display the program -->
                                        <?php if ($currentRefprogramId !== $cascadingKegiatan->refprogram_id): ?>
                                            <tr>
                                                <th colspan="<?= $isAdmin ? 6 : 5 ?>" style="background-color: #16537e; color:white; white-space: normal;"><?= $cascadingKegiatan->refCascadingProgram->refProgram->kode_program ?> - <?= $cascadingKegiatan->refCascadingProgram->refProgram->nama_program ?></th>
                                            </tr>
                                            <?php $currentRefprogramId = $cascadingKegiatan->refprogram_id; ?>
                                        <?php endif; ?>

                                        <!-- Display Kegiatan and group by kegiatan_id -->
                                        <?php if ($currentRefkegiatanId !== $cascadingKegiatan->refkegiatan_id): ?>
                                            <tr>
                                                <?php if ($isAdmin): ?>
                                                    <th style="background-color: #1DE9B6; color:white;">Aksi</th>
                                                <?php endif; ?>
                                                <th style="background-color: #1DE9B6; color:white;"><?= $cascadingKegiatan->refKegiatan->kode_kegiatan ?></th>
                                                <th style="background-color: #1DE9B6; color:white; white-space: normal;"><?= $cascadingKegiatan->refKegiatan->nama_kegiatan ?></th>
                                                <th style="background-color: #1DE9B6; color:white;">Satuan</th>
                                                <th style="background-color: #1DE9B6; color:white;"><?= $cascadingKegiatan->refPeriode->periode ?></th>
                                                <th style="background-color: #1DE9B6; color:white;">Anggaran</th>
                                            </tr>
                                            <?php
                                            $currentRefkegiatanId = $cascadingKegiatan->refkegiatan_id;
                                            // Calculate total anggaran for the current refkegiatan_id
                                            $totalAnggaran = SakipCascadingsubkegiatan::find()
                                                ->where(['refkegiatan_id' => $cascadingKegiatan->refkegiatan_id])
                                                ->andWhere(['refskpd_id' => $cascadingKegiatan->refskpd_id])
                                            ...
                                            <?php if ($isAdmin): ?>
                                                <td class="text-center">
                                                    <?= Html::button('<i class="fas fa-edit"></i>', [
                                                        ...
                                                        'data-url' => Url::to(['update', 'refcascadingkegiatan_id' => $cascadingKegiatan->refcascadingkegiatan_id])
                                                    ]) ?>
                                                </td>
                                            <?php endif; ?>
                                            <th>Sasaran/Indikator</th>
                                            <th style="white-space: normal;"><?= $cascadingKegiatan->uraian_sasarankegiatan ?> - <?= $cascadingKegiatan->uraian_indikatorkegiatan ?></th>
                                            <th><?= $cascadingKegiatan->kegiatan_satuan ?></th>
                                            <th><?= $cascadingKegiatan->kegiatan_target ?></th>
                                        ...
                                        <tr>
                                            <td colspan="<?= $isAdmin ? 6 : 5 ?>">
                                                <div class="accordion" id="accordion<?= $cascadingKegiatan->refcascadingkegiatan_id ?>">
                                                    <div class="accordion-item">
                                                        <h2 class="accordion-header" id="heading<?= $cascadingKegiatan->refcascadingkegiatan_id ?>">
                                                            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse<?= $cascadingKegiatan->refcascadingkegiatan_id ?>" aria-expanded="false" aria-controls="collapse<?= $cascadingKegiatan->refcascadingkegiatan_id ?>">
                                                                Lihat Penjabat SKPD
                                                            </button>
                                                        </h2>
                                                        <div id="collapse<?= $cascadingKegiatan->refcascadingkegiatan_id ?>" class="accordion-collapse collapse" aria-labelledby="heading<?= $cascadingKegiatan->refcascadingkegiatan_id ?>" data-bs-parent="#accordion<?= $cascadingKegiatan->refcascadingkegiatan_id ?>">
                                                            <div class="accordion-body">
                                                                <?php
                                                                $penjabatModels = SakipPenjabatskpdCascadingkegiatan::findAll(['refcascadingkegiatan_id' => $cascadingKegiatan->refcascadingkegiatan_id]);
