    ...
    public function actionIndexEsakip($refperiode_id = null, $refskpd_id = null)
    {
        ...
        foreach (range(1, 4) as $triwulan_id) {
            ...
            foreach ($programList as $program) {
                $indikatorList = SakipIndikatorcascadingprogram::find()
                    ->where([
                ...
                foreach ($indikatorList as $indikator) {
                    $capaian = SakipIndikatorcascadingprogramTriwulan::find()
                        ->where([
        ...
        foreach (range(1, 4) as $triwulan_id) {
            ...
            foreach ($kegiatanList as $kegiatan) {
                $indikatorList = SakipIndikatorcascadingkegiatan::find()
                    ->where([
                ...
                foreach ($indikatorList as $indikator) {
                    $capaian = SakipIndikatorcascadingkegiatanTriwulan::find()
                        ->where([
                            'refindikatorkegiatan_id' => $indikator->refindikatorkegiatan_id,
                            'refcascadingprogram_id' => $kegiatan->refcascadingprogram_id,
                            'refcascadingkegiatan_id' => $kegiatan->refcascadingkegiatan_id,
                            'refskpd_id' => $refskpd_id,
                            'refperiode_id' => $refperiode_id,
                            'reftriwulan_id' => $triwulan_id,
        ...
        foreach (range(1, 4) as $triwulan_id) {
            ...
            foreach ($subkegiatanList as $subkegiatan) {
                $indikatorList = SakipIndikatorcascadingsubkegiatan::find()
                    ->where([
                        'refcascadingsubkegiatan_id' => $subkegiatan->refcascadingsubkegiatan_id,
                        'refperiode_id' => $refperiode_id
                    ])
                    ->all();

                foreach ($indikatorList as $indikator) {
                    $capaian = SakipIndikatorcascadingsubkegiatanTriwulan::find()
                        ->where([
                            'refindikatorsubkegiatan_id' => $indikator->refindikatorsubkegiatan_id,
                            'refcascadingprogram_id' => $subkegiatan->refcascadingprogram_id,
                            'refcascadingkegiatan_id' => $subkegiatan->refcascadingkegiatan_id,
                            'refcascadingsubkegiatan_id' => $subkegiatan->refcascadingsubkegiatan_id,
                            'refskpd_id' => $refskpd_id,
                            'refperiode_id' => $refperiode_id,
                            'reftriwulan_id' => $triwulan_id,
    ...
    public function actionPortalPublikTabulasi($refperiode_id = null, $target_page = null)
    {
        ...
        foreach (range(1, 4) as $triwulan_id) {
            ...
            foreach ($skpdList as $skpd) {
                ...
                $capaianTertinggiSkpd = null;
                $capaianTerendahSkpd = null;
                $indikatorTertinggi = null;
                $indikatorTerendah = null;

                foreach ($indikatorList as $indikator) {
                    $capaianTriwulan = SakipIndikatorsasaranrenstraTriwulan::find()
                        ->where([
                            'refindikatorsasaranrenstra_id' => $indikator->refindikatorsasaranrenstra_id,
                            'refsasaranrenstra_id' => $indikator->refsasaranrenstra_id,
                            'refskpd_id' => $refskpd_id,
                            'refperiode_id' => $refperiode_id,
                            'reftriwulan_id' => $triwulan_id,
                        ])
                    ...
                    if (
                        $capaianTriwulan &&
                        $capaianTriwulan->triwulan_capaian !== null &&
                        (float)$capaianTriwulan->triwulan_capaian > 0
                    ) {
                        $nilai = (float)$capaianTriwulan->triwulan_capaian;

                        // Capaian tertinggi dan indikator tertinggi
                        if ($capaianTertinggiSkpd === null || $nilai > $capaianTertinggiSkpd) {
                            $capaianTertinggiSkpd = $nilai;
                            $indikatorTertinggi = $indikator->uraian_indikatorsasaranrenstra;
                        }

                        // Capaian terendah dan indikator terendah
                        if ($capaianTerendahSkpd === null || $nilai < $capaianTerendahSkpd) {
                            $capaianTerendahSkpd = $nilai;
                            $indikatorTerendah = $indikator->uraian_indikatorsasaranrenstra;
                        }
                    }
                }

                // Update global tertinggi dan terendah
                if (
                    $capaianTertinggiSkpd !== null &&
                    ($nilaiTertinggiGlobal === null || $capaianTertinggiSkpd > $nilaiTertinggiGlobal)
                ) {
                    $nilaiTertinggiGlobal = $capaianTertinggiSkpd;
                    $skpdTertinggi = [
                        'nama' => "Triwulan $triwulan_id - {$skpd->nama_skpd}",
                        'value' => $capaianTertinggiSkpd,
                        'indikator' => $indikatorTertinggi,
                    ];
                }
                ...
                if (
                    $capaianTerendahSkpd !== null &&
                    ($nilaiTerendahGlobal === null || $capaianTerendahSkpd < $nilaiTerendahGlobal)
                ) {
                    $nilaiTerendahGlobal = $capaianTerendahSkpd;
                    $skpdTerendah = [
                        'nama' => "Triwulan $triwulan_id - {$skpd->nama_skpd}",
                        'value' => $capaianTerendahSkpd,
                        'indikator' => $indikatorTerendah,
                    ];
                }
            }
        ...
        foreach ($skpdList as $skpd) {
            ...
            // Fetch cascading programs for the selected SKPD and period
            $programList = SakipCascadingProgram::find()
                ->where(['refskpd_id' => $refskpd_id, 'refperiode_id' => $refperiode_id])
                ->all();

            foreach ($programList as $program) {
                // Fetch indicators related to each cascading program
                $indikatorList = SakipIndikatorcascadingprogram::find()
                    ->where(['refcascadingprogram_id' => $program->refcascadingprogram_id, 'refperiode_id' => $refperiode_id])
                    ->all();

                $indikatorData = [];

                foreach ($indikatorList as $indikator) {
                    // Access the 'uraian_indikatorprogram' from the related 'SakipCascadingProgram' model
                    $uraianIndikator = $program->uraian_indikatorprogram;

                    // Fetch triwulan data for each indicator
                    $triwulanData = SakipIndikatorcascadingprogramTriwulan::find()
                        ->where([
                            'refindikatorprogram_id' => $indikator->refindikatorprogram_id,
                            'refcascadingprogram_id' => $program->refcascadingprogram_id,
                            'refskpd_id' => $refskpd_id,
                    ...
                    $indikatorData[] = [
                        'uraian_indikator' => $uraianIndikator,
                        'triwulan' => $triwulanData,
                        'program_satuan' => $program->program_satuan, // tambahkan field ini
                    ];
                }
                        ...
                        if (
                            ...
                            if ($nilaiTertinggiGlobal === null || $nilai > $nilaiTertinggiGlobal) {
                                ...
                                $programTertinggi = [
                                    'nama' => "Triwulan $triwulan_id - {$skpd->nama_skpd}",
                                    'value' => $nilai,
                                    'indikator' => $program->uraian_indikatorprogram ?? $indikator->uraian_indikatorprogram,
                                ];
                            }
        ...
        foreach ($skpdList as $skpd) {
            ...
            foreach ($kegiatanList as $kegiatan) {
                ...
                foreach ($indikatorList as $indikator) {
                    $triwulanData = SakipIndikatorcascadingkegiatanTriwulan::find()
                        ->where([
                            'refindikatorkegiatan_id' => $indikator->refindikatorkegiatan_id,
                            'refcascadingprogram_id' => $kegiatan->refcascadingprogram_id, // â† ambil dari $kegiatan
                            'refcascadingkegiatan_id' => $kegiatan->refcascadingkegiatan_id,
                            'refskpd_id' => $refskpd_id,
                            'refperiode_id' => $refperiode_id,
                        ])
                    ...
                    $indikatorData[] = [
                        ...
                        'triwulan' => $triwulanData,
                        'kegiatan_satuan' => $kegiatan->kegiatan_satuan, // <== Tambahkan ini
                    ];
                }
                        ...
                        if (
                            ...
                            if ($nilaiTertinggiGlobal === null || $nilai > $nilaiTertinggiGlobal) {
                                ...
                                $kegiatanTertinggi = [
                                    'nama' => "Triwulan $triwulan_id - {$skpd->nama_skpd}",
                                    'value' => $nilai,
                                    'indikator' => $kegiatan->uraian_indikatorkegiatan ?? $indikator->uraian_indikatorkegiatan,
                                ];
                            }
        ...
        foreach ($skpdList as $skpd) {
            ...
            foreach ($subkegiatanList as $subkegiatan) {
                ...
                foreach ($indikatorList as $indikator) {
                    $triwulanData = SakipIndikatorcascadingsubkegiatanTriwulan::find()
                        ->where([
                            'refindikatorsubkegiatan_id' => $indikator->refindikatorsubkegiatan_id,
                            'refcascadingprogram_id' => $subkegiatan->refcascadingprogram_id, // â† ambil dari $kegiatan
                            'refcascadingkegiatan_id' => $subkegiatan->refcascadingkegiatan_id,
                            'refcascadingsubkegiatan_id' => $subkegiatan->refcascadingsubkegiatan_id,
                            'refskpd_id' => $refskpd_id,
                            'refperiode_id' => $refperiode_id,
                    ...
                    $indikatorData[] = [
                        ...
                        'triwulan' => $triwulanData,
                        'subkegiatan_satuan' => $subkegiatan->subkegiatan_satuan, // <== Tambahkan ini
                    ];
                }
