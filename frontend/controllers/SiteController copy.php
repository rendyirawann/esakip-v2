    ...
    public function actionPortalPublikTabulasi($refperiode_id = null, $target_page = null)
    {
                ...
                ->with([
                    'indikators' => function ($q) use ($refperiode_id) {
                        $q->andWhere(['refperiode_id' => $refperiode_id])
                            ->with(['refIndikatorsasaranrenstraTriwulan' => function ($t) use ($refperiode_id) {
                                $t->andWhere(['refperiode_id' => $refperiode_id])
            ...
            foreach ($sasaranList as $sasaran) {
                ...
                foreach ($sasaran->indikators as $indikator) {
                    $indikatorData[] = [
                        ...
                        'triwulan' => $indikator->refIndikatorsasaranrenstraTriwulan,
                    ];
                }
        ...
        foreach ($skpdList as $skpd) {
            ...
            $programList = SakipCascadingProgram::find()
                ->where(['refskpd_id' => $refskpd_id, 'refperiode_id' => $refperiode_id])
                ->with([
                    'indikatorCascadingPrograms' => function ($q) use ($refperiode_id) {
                        $q->andWhere(['refperiode_id' => $refperiode_id])
                            ->with(['refIndikatorCascadingProgramTriwulan' => function ($q) use ($refperiode_id) {
                                $q->andWhere(['refperiode_id' => $refperiode_id])
                                    ->orderBy(['reftriwulan_id' => SORT_ASC]);
                            }]);
                    }
        ...
        foreach ($skpdList as $skpd) {
            ...
            $kegiatanList = SakipCascadingkegiatan::find()
                ->where(['refskpd_id' => $refskpd_id, 'refperiode_id' => $refperiode_id])
                ->with([
                    'indikatorCascadingKegiatan' => function ($q) use ($refperiode_id) {
                        $q->andWhere(['refperiode_id' => $refperiode_id])
                            ->with(['refIndikatorCascadingKegiatanTriwulan' => function ($q) use ($refperiode_id) {
                                $q->andWhere(['refperiode_id' => $refperiode_id])
                                    ->orderBy(['reftriwulan_id' => SORT_ASC]);
                            }]);
                    }
        ...
        foreach ($skpdList as $skpd) {
            ...
            $subkegiatanList = SakipCascadingsubkegiatan::find()
                ...
                ->with([
                    'refIndikatorcascadingsubkegiatan' => function ($q) use ($refperiode_id) {
                        $q->andWhere(['refperiode_id' => $refperiode_id])
                            ->with(['refIndikatorCascadingSubkegiatanTriwulan' => function ($q) use ($refperiode_id) {
                                $q->andWhere(['refperiode_id' => $refperiode_id])
                                    ->orderBy(['reftriwulan_id' => SORT_ASC]);
                            }]);
                    }
