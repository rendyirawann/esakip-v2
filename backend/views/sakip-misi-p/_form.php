...
$this->registerJs("
    ...
    $(document).ready(function() {
        ...
        $('form#datamisip').off('submit').on('submit', function(e) {
...
<div class="data-misi-p-form">
    ...
    <?= $form->field($model, 'uraian_misi_p')->widget(CKEditor::className(), [
        'options' => ['rows' => 5],
        'preset' => 'full'
    ]); ?>
    ...
    <?= $form->field($model, 'refperiode_id')->dropDownList(
        ...
        [
            ...
            $.post("' . Url::to(['sakip-misi-p/lists', 'id' => '']) . '"+periodeId, function(data) {
                var visiDropdown = $("select#sakipmisip-refvisi_p_id");
    ...
    <?= $form->field($model, 'refvisi_p_id')->dropDownList(
        ArrayHelper::map(
            ...
            'refvisi_p_id',
            function ($model) {
                return Html::decode($model->uraian_visi_p) . " - " . Html::encode($model->refperiode_id); // Decode & encode data
            }
        ),
        [
            'prompt' => 'Pilih Uraian Visi Perubahan Terkait',
            'options' => [
                $model->refvisi_p_id => ['Selected' => true], // Menandai yang terpilih
            ],
        ]
    ...
    <?= $form->field($model, 'misi_p_isaktif')->radioList(
