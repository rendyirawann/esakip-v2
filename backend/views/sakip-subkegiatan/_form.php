...
$this->registerJs("
    ...
    function saveFormData() {
        $('#datasubkegiatan').find(':input').each(function() {
            var input = $(this);
    ...
    $(document).ready(function() {
        ...
        $(document).on('shown.bs.modal', '#createModal', function() {
            ...
            $('#datasubkegiatan').find(':radio').each(function() {
                var input = $(this);
        ...
        $('#datasubkegiatan').on('input', function() {
            saveFormData(); // Save form data on input change
        });

        $('#datasubkegiatan').off('submit').on('submit', function(e) {
            e.preventDefault();
...
<div class="sakip-subkegiatan-form">
    ...
    <?= $form->field($model, 'kode_subkegiatan')->textInput(['maxlength' => true]) ?>

    <?= $form->field($model, 'nama_subkegiatan')->textarea(['rows' => 6]) ?>
    ...
    <?= $form->field($model, 'refurusan_id')->dropDownList(
        ...
        [
            ...
            $.post("' . Url::to(['sakip-subkegiatan/lists', 'id' => '']) . '"+urusanId, function(data) {
                var bidangDropdown = $("select#sakipsubkegiatan-refbidang_id");
                ...
                $("select#sakipsubkegiatan-refprogram_id").html("<option>Pilih Program</option>").val(""); // Reset program
                $("select#sakipsubkegiatan-refkegiatan_id").html("<option>Pilih Kegiatan</option>").val(""); // Reset kegiatan
            });'
        ]
    ...
    <?= $form->field($model, 'refbidang_id')->dropDownList(
        ...
        [
            ...
            $.post("' . Url::to(['sakip-subkegiatan/list-program', 'id' => '']) . '"+bidangId, function(data) {
                var programDropdown = $("select#sakipsubkegiatan-refprogram_id"); // pastikan ID ini sesuai
    ...
    <?= $form->field($model, 'refprogram_id')->dropDownList(
        ...
        [
            ...
            $.post("' . Url::to(['sakip-subkegiatan/list-kegiatan', 'id' => '']) . '"+programId, function(data) {
                var kegiatanDropdown = $("select#sakipsubkegiatan-refkegiatan_id");
                kegiatanDropdown.html(data).val(""); // Reset kegiatan dengan data baru
                kegiatanDropdown.prepend("<option value=\"\">Pilih Kegiatan</option>"); // Tambahkan prompt
            });'
        ]
    ...
    <?= $form->field($model, 'refkegiatan_id')->dropDownList(
        ArrayHelper::map(
            SakipKegiatan::find()->where(['refprogram_id' => $model->refprogram_id])->orderBy('kode_kegiatan ASC')->all(),
            'refkegiatan_id',
            function ($model) {
                return $model->kode_kegiatan . ' - ' . $model->nama_kegiatan;
            }
        ),
        [
    ...
    <?= $form->field($model, 'subkegiatan_isaktif')->radioList(
