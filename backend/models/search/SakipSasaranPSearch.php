<?php

namespace backend\models\search;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use backend\models\SakipSasaranP;

/**
 * SakipSasaranSearch represents the model behind the search form of `backend\models\SakipSasaran`.
 */
class SakipSasaranPSearch extends SakipSasaranP
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['refsasaran_p_id', 'refperiode_5tahun_id', 'refvisi_p_id', 'refmisi_p_id', 'reftujuan_p_id'], 'integer'],
            [['uraian_sasaran_p', 'sasaran_p_isaktif'], 'safe'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = SakipSasaranP::find();

        // add conditions that should always apply here
        $query->with(['visi', 'misi', 'tujuan', 'periode']);

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'refsasaran_p_id' => $this->refsasaran_p_id,
            'refperiode_5tahun_id' => $this->refperiode_5tahun_id,
            'refvisi_p_id' => $this->refvisi_p_id,
            'refmisi_p_id' => $this->refmisi_p_id,
            'reftujuan_p_id' => $this->reftujuan_p_id,
        ]);

        $query->andFilterWhere(['like', 'uraian_sasaran_p', $this->uraian_sasaran_p])
            ->andFilterWhere(['like', 'sasaran_p_isaktif', $this->sasaran_p_isaktif]);

        return $dataProvider;
    }
}
